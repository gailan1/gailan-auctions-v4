...
async function open(s){
  session=s;
  const r=await sb.from("profiles").select("*").eq("id",s.user.id).limit(1);
  if(r.error){$("loginMsg").textContent="تعذر تحميل بيانات المستخدم: "+r.error.message;return}
  if(!r.data||r.data.length===0){$("loginMsg").textContent="لم يتم العثور على بيانات المستخدم";return}
  profile=r.data[0];
  if(profile.active===false){await sb.auth.signOut();alert("الحساب متوقف");return}
